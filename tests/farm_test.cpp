...
TEST_CASE("it returns the symbol for a single soil plot with player") {
    Player player;
    Farm farm(1, 1, &player);
...
TEST_CASE("it grows carrots correctly each day") {
    Player player;
    Farm farm(1, 2, &player);
    Carrot *carrot = new Carrot();
    farm.plant(0, 1, carrot);
    ...
    REQUIRE(farm.get_symbol(0, 1) == "V");
    farm.next_day();
    REQUIRE(farm.get_symbol(0, 1) == "W");
}

TEST_CASE("it allows harvest when carrot age > 0") {
    Player player;
    Farm farm(1, 2, &player);
    Carrot *carrot = new Carrot();
    farm.plant(0, 1, carrot);
...
TEST_CASE("it increments the global day count") {
    Player player;
    Farm farm(2, 2, &player);

    REQUIRE(farm.get_day() == 1);
    farm.next_day();
    REQUIRE(farm.get_day() == 2);
    farm.next_day();
    REQUIRE(farm.get_day() == 3);
}